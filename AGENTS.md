...
官方远端为 `gitea`（`https://gitea.xxx.com:11111/example/Engram.git`，私有）。开发、合并、发版均在本地完成后按用户明确指示推送 gitea；不经批准不推其他远端，不强推或改写远端历史。
...
4. 镜像地址固定三层路径 `gitea.xxx.com:11111/example/engram/engram:<版本>`（两层 `owner/image` 形式 NAS 拉取异常，勿改回）。

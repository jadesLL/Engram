...
- 设置页 4 大类：账户与外观、连接与同步、Agent 接入、数据与存储；**大类即整页**——每个大类一页平铺展示全部分组，分组是带头部色带（图标 + 标题 + 说明 + 状态徽标）的常开卡片，组间边界清晰；左侧二级锚点只做滚动定位与反向高亮（可跨大类点击，滚到底自动高亮最后一组），「危险操作」以红色分组收尾；窄屏下大类切换为两行胶囊导航

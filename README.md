...
- **回收站**（软删除，Agent 经 MCP `delete_page` / CLI `pages delete` 删除同样只入回收站）、标签、本页关联（双链邻居/实体关系）、沉浸阅读（**打开页面默认进入阅读视图**，需编辑时点「返回编辑」；正文字号 1px 连续可调，8–48px，点字号可直接下拉选择；**正文宽度按可用区百分比选档**（50%–100%，默认 70%），点工具栏的百分比按钮即选；标题左侧箭头可按节收放（H2–H4，收起时连同子节一起折叠，状态仅当前会话））

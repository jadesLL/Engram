...
- **单列转录、执行记录可展开**：对话流与 dsh 同构——用户消息、思考过程、Agent 正文、工具调用（检索 / 读页 / 写页…）按发生顺序排成一条单列流；工具调用默认收起为一行摘要（工具名 + 参数 + 状态），点开即看完整参数与完整结果（等宽滚动，不截断），失败自动展开；可随时「停止」，可把一轮对话「沉淀到原始资料」供后续提炼
- **子代理派活看得见**：Agent 把活派给子代理（dsh 的 `subagent` / `subagent_fork` / `workflow` / `ralph` 子会话）时，对话流里会出现一张**子代理卡**——标签（模型给的 3-5 词任务名）、运行状态与用时秒表、**委托任务原文**、子代理自己的**过程**（它调了哪些工具、参数摘要、成败与时间点）、以及**产出**（最后一条正文；收工时以 dsh 的 `subagent.finished` 定稿）；子代理再派子代理会嵌在父卡里显示。派它的那次工具调用并进同一张卡，不会重复两行；本轮结束时仍在后台跑的子代理标「后台运行中」。为此内置 Agent 的约定里写明：委派子代理默认同步等结果（`run_in_background: false`），除非用户明确要并行后台作业
- **只要在跑就一直显示**：一轮回复期间，抽屉标题右侧常驻「回复中 + 用时」，转录区底部有一条**贴底的运行状态条**（当前在干什么：子代理 / 工具 / 步骤，逐秒跳的用时），滚到哪一段都看得见，收口即消失；抽屉关着（或页面刷新后）时图标栏的 ✨ 会有呼吸动效 + 脉冲点，跑完换成「新回复」小红点——任何时候都知道它还在不在干活
- **思考过程流式显示**：模型的推理内容不再丢弃，单独成块（弱化虚线框 + 灯泡图标），默认展开、边生成边长出来；dsh 的事件是「按步提交」的，所以思考过程按会话事件里保留的模型原始增量与真实节奏回放（单步封顶 2 秒，正文等这一步回放完再显示，避免思考与答案错序）；本轮结束后自动收起为一行（`思考 N 秒`），点标题可随时展开看全文，手动点过就以手动为准；思考段只进对话流，不进模型上下文，也不随「沉淀对话」写进知识库

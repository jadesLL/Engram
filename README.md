...
- **语义转换，不是格式搬运**：点「转为 Markdown」把原件**按内容语义重写**成人类可读的 Markdown——表格重建、按体裁重组信息（访谈按对象/现状/痛点/关注点）、数字与人名逐字保留、不补内容，文末固定「待确认」与「原件信息」。产物落在 `收集箱/转换结果/`，可在应用内审阅（带来源 frontmatter），此时仍不进知识库。Office / PDF / 文本由服务端直接转（长文按段落分块，超上限会明确标注只覆盖部分）；图片与音视频走 Agent 通道（视觉识别 / 转写）；压缩包等给出可读的说明而不是假装能转。
...
- **入库才是纳入知识库**：确认无误后点「入库」，产物按原件名称保存为 Markdown，直接复制进 `原始资料/` 根目录并登记为页面；同名自动加序号。从此可在原始资料清单中找到，也可检索、引用、作为逐字证据来源。旧版曾落入 `原始资料/收集箱/` 的文件会在启动时迁到根目录，保留页面 ID。原件与产物都留在收集箱，可随时再转一版。

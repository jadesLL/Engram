...
- **Markdown 编辑器**（Vditor）：即时渲染 / 分屏切换、大纲、全屏、`Ctrl+S`、中文输入法组字保护；图片粘贴自动入 `assets/`；顶部条面包屑 + 保存状态 pill（有未保存改动时点击即保存，Ctrl+S 不变）+ 按文件的「自动保存」开关（默认开，关闭后仅手动保存/Ctrl+S）、标题与正文列对齐、工具栏按「格式/段落/插入/历史/视图」分组的统一描边图标、底部状态栏（字数 / 模式 / 来源 / 图谱）；深色模式下原生下拉/滚动条随主题变色
...
- **UI 2.0（2026-09）**：暖灰中性色 + 统一品牌蓝（浅 `#0f6cbd` / 深 `#5aa9e6`）；圆角放大（控件 6px / 卡片 11px），输入框与按钮聚焦改为柔和光圈；**欢迎页**改为时间问候 + 库统计 + 快捷入口卡（新建/搜索/图谱/问 Agent）+ 最近编辑列表；**搜索页**重做命令面板式（48px 搜索盒、范围筛选 chips、结果卡片带类型徽标与关键词高亮、显示耗时）；**设置页**分组标题外置可折叠 + 主题切换改分段控件；**登录页**品牌渐变卡；编辑器正文栏宽收敛 720px，阅读视图顶栏 9 控件收为 5 个（行距/编号/目录收进「显示」菜单）

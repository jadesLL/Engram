...
- **应用内自更新**（设置 → 软件更新）：Docker 版网页一键拉取新镜像并自动重建容器（进度实时滚动、失败自动回滚）；桌面端同页检测并下载 exe 安装包覆盖安装；进入应用自动检测新版本（红点 + 提醒）；更新源与令牌全部在设置页配置（存数据目录 `.env`，接口不回明文），私有化部署可指向自己的 Registry/Gitea

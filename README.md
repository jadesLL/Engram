...
- **右侧并排 / 满窗全屏**：抽屉可一键在「右侧并排」与「满窗铺满内容区」之间切换（满窗时左侧图标栏与文件树让位给对话，桌面端标题栏与窗口按钮照常可用），选择记在本地，下次打开沿用；满窗下按 `Esc` 或点「收回右侧」即可还原
...
- **NAS Docker**：单容器 + ONLYOFFICE；**Android APP** 远程客户端；**Windows 桌面端**（Electron 内嵌本地后端，托盘驻留、自动更新）

...
Docker 版同样支持**跟主分支不发版更新**：设置 → 连接与同步 → 服务器更新 → 「更新通道」选 `main`，之后每次 main 推送 CI 都会重推 `:main` 镜像，点「立即更新」即拿到主分支最新代码（版本号不变，看提交号判断）；发版前的测试就靠这条通道，验证完切回 `latest`。默认通道 `latest` 只跟正式发版。

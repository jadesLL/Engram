...
- **Markdown 编辑器**（Vditor）：即时渲染 / 分屏切换、大纲、全屏、`Ctrl+S`、中文输入法组字保护；图片粘贴自动入 `assets/`；扁平化编辑页（灰底正文 + 46px 白顶栏 + 34px 白状态栏，无悬浮卡片）——顶栏是 `Wiki / 分区 / 标题` 面包屑 + 常驻保存态（绿点 + 文案，有未保存改动时点击即保存，Ctrl+S 不变）+ 按文件的「自动保存」开关（默认开，关闭后仅手动保存/Ctrl+S）+ 「正文宽度」百分比按钮；标题 30px，文档列宽按**正文可用区的百分比**（默认 70%，与阅读视图共用同一份偏好，改一处两处都变），页头/正文/工具栏左缘始终对齐；类型与标签是圆角 pill（类型软色强调、`+ 标签` 虚线按钮点开输入框）；工具栏是 fit-content 白卡片，按「格式 / 段落 / 插入 / 历史」四段分组（双链、源码切换、全屏、大纲、沉浸阅读收在末段）；底部状态栏（字数 / 模式 / 来源 / 图谱）；深色模式下原生下拉/滚动条随主题变色

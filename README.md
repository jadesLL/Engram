...
- **图片资产清理**（设置 → 存储空间）：列出**未归属**（历史遗留的散图，用户已不能单独上传图片）与**未被引用**（正文里已删掉引用的残留）两类，可逐张删除或全部清理；未归属图片还能选一个页面「挂载」，服务端把它移到该页面名下并把引用补进正文末尾
...
2. 日常双击桌面「Engram」直接启动（Engram 品牌图标，无更新窗口）；更新走应用内 设置 → 软件更新 → 「检查更新」（增量拉源码重建重启），或手动运行 `main/scripts/update-from-source.ps1`——合 main 即更新，无需等发版。源码模式的启动程序是 Electron 官方 `electron.exe`（资源管理器/任务栏里显示的是 Electron 原子图标），构建与更新时会在同目录生成一份带 Engram 图标的 `Engram.exe`，桌面（及已存在的开始菜单）快捷方式指向它；**桌面图标丢了或显示不对**时，用 设置 → 软件更新 → 桌面端 → 「重建桌面快捷方式」一键重建

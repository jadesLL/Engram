...
图标栏「知识图谱」下方进入收集箱：**拖入任意格式，单文件大小不限**（流式写入，不设固定单文件上限），落盘即多端同步，同名不覆盖、自动加序号。网页可先从浏览器另存为 HTML 或 PDF 后拖入；收集箱目前没有粘贴网址并自动抓取网页的入口。
...
- **入库才是纳入知识库**：确认无误后点「入库」，产物复制进 `原始资料/收集箱/` 并登记为页面——从此可检索、可引用、可作逐字证据来源。原件与产物都留在收集箱，可随时再转一版。
...
│   ├── 原始资料/        # 上传的资料与对话沉积；文本层提取后可被检索（图片不能单独上传）；收集箱入库的产物也落在这里（原始资料/收集箱/）

...
- **来源证据抽屉**：每个页面的论断可展开查看支撑它的**原子事实 + 原文逐字引文**（含时间线证据），页面综合状态（人工已修改 / 新资料待综合 / 已验证）一目了然——AI 写的内容可溯源、可核对
- **本页关联**：双链邻居（→/←）、语义相似页（≈）、实体标签关系，发现「和这个页面相关的还有哪些」
- **阅读模式**：沉浸式预览
...
死链检测 · 疑似重复 · 内容矛盾 · 待丰富 · 信息过期 · 来源单一 · 待补章节 · 实体歧义（+实体升级建议），产出一份可操作的整理报告。

### 📋 整理报告与决策

报告页按「**待决策 → 待入库 → 提醒**」三区组织：

- **待决策**：选择题卡片（死链怎么处理 / 重复页面合并谁 / 矛盾以谁为准），同对象的多个问题聚合成一张卡；按类型分组折叠，支持批量处理预览；破坏性合并需二次确认
...
- **实体歧义审核**：同名/近似实体身份混淆扫描——同一对实体只问一次（A→B 与 B→A 自动归一，历史镜像记录启动时合并清理）；答「是」后在选项下方内联选择保留哪一侧或自定义最终名称，无二次弹窗；另有重命名澄清 / 标记误报
...
- **应用内自更新**（设置 → 软件更新）：Docker 版网页一键拉取新镜像并自动重建容器（进度实时滚动、失败自动回滚）；桌面端同页检测并下载 exe 安装包覆盖安装；进入应用自动检测新版本（红点 + 提醒）；更新源只需粘贴远端仓库整条地址自动识别，访问凭据支持访问令牌/用户名密码二选一（明文回显所见即所得，存数据目录 `.env`）；镜像源自动从当前容器推导，自定义镜像源等高级选项默认折叠；更新检测的镜像对比经宿主机 Docker daemon 代查（容器网络受限如 IPv6-only 仓库照样可用），失败原因完整展示网络层根因（如 `fetch failed ← getaddrinfo ENOTFOUND`）而非一句 `fetch failed`；镜像版本比对由宿主机 Docker daemon 代查（与拉取镜像同一网络路径），容器自身无 IPv6 或出站受限时检查更新依然可用；更新错误逐层展开底层原因（如 DNS 解析失败），不再只显示 fetch failed
- 设置页 10 大面板：账户、模型配置（对话/向量/视觉三类+测试+重建索引）、提炼看板、自动化、MCP、桌面端连接、软件更新、飞书、存储空间、数据管理
- **模型用量统计**：按操作类型汇总调用次数、**缓存命中率**（总命中率 / 供应商前缀命中率）、缓存读取 token、重试次数，LLM 成本看得见

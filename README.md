...
- **右侧悬浮卡片 / 满窗全屏**：非满窗档是一张**浮在右侧的毛玻璃卡片**——与左侧文件树同一套材质（圆角 8px、四周留 8px 露出底色、投影），正文让出它的宽度，聊天时不遮正文；打开与收起从右缘滑入滑出（轻微缩放 + 淡入，与左侧栏同一套节奏），宽度同步收放，正文让位跟着动画一起走、不会先跳一下。点标题栏的满窗按钮即在「悬浮卡片」与「满窗铺满内容区」之间切换（满窗只铺满正文区，左侧图标栏与文件树保持可见可用；桌面端标题栏与窗口按钮照常可用），选择记在本地，下次打开沿用；满窗下按 `Esc` 或点「收回右侧」即可还原

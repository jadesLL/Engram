...
- **自建悬停提示（不再用系统原生 title）**：全站提示统一走自建气泡，支持「标题 + 说明 + 快捷键」三段式；气泡**先测量真实尺寸再择向**，四边留 8px 安全区、箭头钉住被说明对象，任何情况下不越界、不压住被说明对象。默认**严格避让**：对附近真实内容块做碰撞检测，近处没有空位就外移到旁边的空白处并用虚线指向对象；设置 → 账户与外观 可切「就近优先」（允许轻微遮挡，观感更接近系统原生提示）。图谱节点用**光标跟随**式提示（贴着鼠标走、不遮节点），密钥字段、图谱工具条、返回轨迹等原本走系统 `title` 的地方已全部换成自建提示

...
- **本页关联**：双链邻居（→/←）、语义相似页（≈）、实体标签关系，发现「和这个页面相关的还有哪些」；手机端默认折叠为一行计数摘要（多关联页铺开会占约 1/3 屏），点击展开，桌面端默认展开。**手机端编辑页同样紧凑**：页头操作区（类型/标签/AI 整理等）默认折叠为一行摘要，编辑器工具栏精简为单行高频按钮，正文占据大半屏；桌面端不变
- **阅读模式**：沉浸式预览

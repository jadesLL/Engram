...
- **满窗下切内容自动最小化**：满窗时点「设置」「实体页」「搜索」「图谱」或侧栏任意页面（双链跳转、搜索结果、返回轨迹同样算），抽屉自动**最小化**收进图标栏的 ✨——不占右侧、正文立刻占满整屏，不会换一条卡片继续挡着新内容；刚最小化的 4 秒里右下角状态胶囊提示「已最小化 · 点此继续对话」，点它或点 ✨ 即回到最小化前那个形态（悬浮形态与已关闭状态不受影响）
...
- **应用内自更新**：Docker 网页一键升级；桌面端全自动更新；**同步成员端可远程更新中枢**——桌面端绑定多端同步后，设置 → 连接与同步 → 软件更新直接对所连服务器检查/拉镜像/重建容器（SSE 进度 + 代等恢复 + 失败自动回滚），无需登录服务器网页，同步连接即授权（更新源凭据读写仍仅服务器 owner）
- 设置页 4 大类：账户与外观、连接与同步、Agent 接入、数据与存储；**大类即整页**——每个大类一页平铺展示全部分组，分组是带头部色带（图标 + 标题 + 说明 + 状态徽标）的独立卡片，组间边界清晰；**每张卡片都可折叠**（点色带箭头或空白处收起/展开，折叠状态按分组记忆，锚点跳转自动展开目标分组）；左侧二级锚点只做滚动定位与反向高亮（可跨大类点击，滚到底自动高亮最后一组），「危险操作」以红色分组收尾；窄屏下大类切换为两行胶囊导航；**侧栏拉宽挤压内容列时**，设置行自动转单列、控件换行，任何宽度下都不会被裁切
...
3. 更新：应用内 设置 → 连接与同步 → 软件更新 → 「检查更新」，自动下载并静默安装
...
4. 源码模式怎么确认「更新到了哪一版」：设置 → 账户与外观 → 连接与版本（高级，默认收起）→「应用版本」显示 `版本号 · 提交号 · 提交日期`（工作区有未提交改动时提交号带 `-dirty`），软件更新页右上角同样带提交号，「检查更新」显示本地 → 远端提交号对比。**版本号只在正式发版时变，提交号随每次更新变**，以提交号判断是否已更新到最新代码（与 Docker 镜像、安装包的版本号语义一致）
5. 卸载：应用内 设置 → 连接与同步 → 软件更新 → 「卸载 Engram」，自动停止应用、删除桌面快捷方式与整个安装目录；知识库数据 `%APPDATA%\@engram\desktop` 默认保留，确认不要可勾选一并删除。也可手动运行 `main/scripts/uninstall-engram.ps1`。全局 pnpm 不受影响（若安装时全局装不上、退到了 `%LOCALAPPDATA%\engram\pnpm-home`，它随安装目录一起删除）

**Docker（服务器 / NAS）**：见下方「快速开始（Docker 部署）」；更新可在 设置 → 连接与同步 → 软件更新 一键拉镜像重建（需挂载 docker.sock）。镜像构建期会把提交号烤入 `/app/GIT_SHA`（release.yml 自动传，本地构建可 `ENGRAM_GIT_SHA=$(git rev-parse HEAD) docker compose up -d --build`），因此设置 → 账户与外观 → 连接与版本（高级，默认收起）→「应用版本」显示 `版本号 · 提交号`，一眼能看出当前跑的是哪次构建。

Docker 版同样支持**跟主分支不发版更新**：设置 → 连接与同步 → 软件更新 → 「更新通道」选 `main`，之后每次 main 推送 CI 都会重推 `:main` 镜像，点「立即更新」即拿到主分支最新代码（版本号不变，看提交号判断）；发版前的测试就靠这条通道，验证完切回 `latest`。默认通道 `latest` 只跟正式发版。

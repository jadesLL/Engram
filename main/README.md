...
- 🖼 **PDF / 图片**：PDF.js 在线查看、缩放、缩略图与搜索；图片支持缩放、拖动和旋转，文字型 PDF 本地提取，扫描页和图片按需调用独立文档识别模型
...
> 设置页单独配置“文档识别”模型。每批自动 OCR 最多 100 页，超出或失败时会明确标记为部分完成。
...
文档识别模型与对话模型相互独立。内置通义 `qwen3.5-ocr` 预设，也可以配置支持
OpenAI 兼容图片输入的自定义模型；只有图片和 PDF 中没有足够内嵌文字的页面会发送给该模型。

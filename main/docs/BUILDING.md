...
**NAS 上的应用内一键更新**：模板已挂 `/var/run/docker.sock`，启动后在网页 设置 → 连接与同步 → 更新源配置 填一次即可（配置落在 `/data/.env`，容器重建不丢）：
...
3. `docker-compose.pull.yml` / `docker-compose.nas.yml` 里的 `/var/run/docker.sock` 挂载是**应用内自更新**（设置 → 连接与同步 → 服务器更新，网页一键拉新镜像重建容器）所需；不需要该功能可删掉这行。
...
Docker 部署在网页「设置 → 连接与同步 → 服务器更新」一键更新（拉目标 tag 镜像 → switcher 容器接管重建 → 失败自动回滚）；桌面端在「桌面端更新」分组下载新 exe 覆盖安装。更新源与令牌在设置页配置，存数据目录 `.env`。机制与安全说明见 [`GITEA-CI.md`](./GITEA-CI.md) 的「应用内自更新」章节。

**更新通道**（设置 → 连接与同步 → 服务器更新）：`latest`（默认）跟正式发版线；`main` 跟主分支滚动构建，合 main 即更新、不发版即可测试。发版前想提早在 Docker 上验证最新代码，就把测试部署切到 `main`，验证完切回 `latest`。

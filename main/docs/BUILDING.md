...
> 设置页「应用版本」显示 `版本号 · 提交号 [· 提交日期]`，版本与更新页右上角与源码模式「检查更新」同样带提交号。
...
Docker 部署在网页「设置 → 版本与更新 → 服务器更新」一键更新（拉目标 tag 镜像 → switcher 容器接管重建 → 失败自动回滚）；桌面端在「桌面端更新」分组下载新 exe 覆盖安装。更新源与令牌在设置页配置，存数据目录 `.env`。机制与安全说明见 [`GITEA-CI.md`](./GITEA-CI.md) 的「应用内自更新」章节。

**更新通道**（设置 → 版本与更新 → 服务器更新）：`latest`（默认）跟正式发版线；`main` 跟主分支滚动构建，合 main 即更新、不发版即可测试。发版前想提早在 Docker 上验证最新代码，就把测试部署切到 `main`，验证完切回 `latest`。

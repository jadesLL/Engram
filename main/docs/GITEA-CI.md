...
- **检查更新**：比对 Gitea 最新 Release 版本号 + Registry 目标 tag 的 digest（两者取或）。`main` 通道下 Release 版本号与本地相同，故实际由 digest 驱动，提示为「主分支镜像有更新」。进入应用约 3 秒后自动检测一次，之后自适应退避复查（连续没发现更新时 2→5→10→30→60 分钟逐级拉长，有更新待处理时每分钟复查；发现更新或切回前台/网络恢复立刻回到最短间隔），有新版本时侧栏设置按钮出现红点、顶部更新提示条弹出更新内容。
- **一键更新（Docker）**：顶部更新提示条的「立即更新」或设置页的「立即更新」都直接执行下面这套流程（不再跳转/二次确认）：拉取目标 tag（默认 `latest`，可切 `main`）镜像 → 用旧镜像临时起 switcher 容器接管 → 旧容器改名 `engram-old` → 按原容器配置（端口/卷/网络/环境变量全保留）创建新容器 → 停旧起新 → 等新容器健康（最长 180s）→ 健康则删旧容器；**新容器起不来则自动回滚**重启旧容器。前端提交后轮询 `/health` 等新容器起来再自动刷新页面。
- **桌面端更新**：顶部更新提示条的「立即下载并安装」或设置页的「下载并安装」都是全自动：下载 Release 的 exe 安装包（带进度条）→ 静默安装覆盖 → 应用自动重启；源码模式则走「立即更新并重启」（增量拉源码 + 重建 + 自动重启）。

...
| 实体页综合 | Page Compose、Page Verify、Manual Merge Check | 汇集有效贡献、版本哈希、隐藏边界、失败回退、证据接口与队列去重 |
...
- 自动新建概念或实体页满足其一即可：两个不同 `原始资料/` 路径各自提供有效事实；或单一来源提供至少两条有效事实，且候选非低置信度、无身份歧义并通过最终验证。
- 同一路径的新版本仍只计一个来源。
- 引文必须逐字存在于对应原文上下文。
- 实体页固定为 `当前理解 -> 相关页面 -> 时间线`；概念页不强制时间线。
- 实体页正文由全部 active 来源事实统一综合，不按来源或入库时间拼接；时间线只接收证据明确支持的带日期状态变化。
- 页面级综合失败或人工修改无法无损合并时，保留上一版有效正文并生成待处理报告。

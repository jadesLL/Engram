...
Windows 桌面客户端，内嵌完整后端，所有数据保存在本机，无需服务器即可使用。需要多设备协作时，在「设置 → 多端同步 → 同步群组」加入同步群组，各端仍保留完整本地库并支持离线工作。
...
4. **开机自启（可选）**：设置 → 本机应用 → 桌面端应用 →「开机自启」打开后，登录 Windows 会**静默启动到系统托盘**——只拉起内嵌后端与托盘图标，**不弹主窗口**；双击托盘图标（或桌面快捷方式）即打开主界面。托盘右键菜单里也有同一个开关。启动项写在注册表 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 的 `Engram` 值（任务管理器「启动」页可见，可在那儿禁用）；便携版不提供该项（运行时目录每次启动都在变）。
5. 数据仓库位置与本地服务端口可在「设置 → 知识库数据 → 存储位置」中调整。

## 软件更新

更新源在 Web 端「设置 → 版本与更新 → 更新源配置」配置（远端仓库地址 + 可选凭据），保存在数据目录 `.env`。
...
- **源码模式的自动检查**：与安装包形态同一套节奏（启动 4 秒首查 + 自适应退避 + 回前台/唤醒补查）。检查走两级探测：先 `git ls-remote` 问远端分支的提交号，只有它和本地 `origin/<分支>` 不一致才真的 `git fetch` 增量对象，因此绝大多数轮次只有一个网络往返。落后时标题栏出现一枚绿底更新图标（**不自动弹面板、也不弹系统通知**，鼠标扫到才展开），点展开面板的「立即更新并重启」即增量拉源码、重建并自动重启（过程中有置顶进度窗）。
- **一键更新入口**：应用内更新入口是一枚绿底图标（桌面端在标题栏应用名右侧，网页端在右上角）。检测到更新不再自动弹出面板；鼠标扫到图标展开，移开约 0.3 秒收起，点一下也能开合。面板主按钮直接执行更新，不再只是跳设置页——源码模式「立即更新并重启」、安装包形态「立即下载并安装」（下载带百分比）、服务端/浏览器形态「立即更新」（提交服务端更新并等新容器起来后自动刷新页面）。进度与失败原因就地显示在面板里；「稍后提醒」（只收起面板，图标留着继续提示）与「查看详情」（跳设置页看完整信息）保留为次级入口。
...
- **卸载（仅源码安装形态）**：设置 → 本机应用 → 卸载 Engram——停止应用并删除桌面快捷方式与整个安装目录（源码、便携运行时、Electron），知识库数据默认保留，可勾选一并删除；安装包形态请在系统「添加或删除程序」中卸载。它独立成「本机应用」里的一个分组（与「桌面端应用」「版本信息」同一类）：删的是这台机器上的安装目录、不是知识数据，所以不再和清库 / 清日志挤在「危险操作」里；分组仍保留红色危险样式、操作不可撤销，但只有清库 / 清日志（仍在 设置 → 知识库数据 → 危险操作）需要登录密码，卸载是本机动作、走应用内确认弹窗。
...
- **桌面快捷方式（源码模式）**：源码模式的启动程序是 Electron 官方 `electron.exe`——资源管理器与任务栏里显示的是 Electron 原子图标。构建/更新时由 `desktop/scripts/ensure-branded-exe.js` 在同一运行时目录复制一份带 Engram 图标的 `Engram.exe`（rcedit 打 `desktop/build/icon.ico`，`electron.exe` 原样保留，Electron 升级或重装依赖后按 mtime 自动重做），桌面快捷方式（以及已存在、且属于本安装的开始菜单快捷方式）指向它。桌面图标丢失或显示不对时，用 设置 → 本机应用 → 桌面端应用 → 「重建桌面快捷方式」重建；若 `Engram.exe` 正在运行（Windows 锁文件）无法覆盖，会提示退出后重试，更新脚本下次会自动补上。

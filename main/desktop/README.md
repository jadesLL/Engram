...
4. **开机自启（可选）**：设置 → 连接与同步 → 桌面端应用 →「开机自启」打开后，登录 Windows 会**静默启动到系统托盘**——只拉起内嵌后端与托盘图标，**不弹主窗口**；双击托盘图标（或桌面快捷方式）即打开主界面。托盘右键菜单里也有同一个开关。启动项写在注册表 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 的 `Engram` 值（任务管理器「启动」页可见，可在那儿禁用）；便携版不提供该项（运行时目录每次启动都在变）。
5. 数据仓库位置与本地服务端口可在「设置 → 数据与存储 → 存储位置」中调整。

## 软件更新

更新源在 Web 端「设置 → 连接与同步 → 更新源配置」配置（远端仓库地址 + 可选凭据），保存在数据目录 `.env`。
...
- **卸载（仅源码安装形态）**：设置 → 数据与存储 → 危险操作 → 「卸载 Engram」——停止应用并删除桌面快捷方式与整个安装目录（源码、便携运行时、Electron），知识库数据默认保留，可勾选一并删除；安装包形态请在系统「添加或删除程序」中卸载。分组为什么在「危险操作」：它和清库、清日志一样不可撤销，所以统一放在设置页最后一个分类；这三项里只有清库/清日志需要登录密码，卸载是本机动作、走应用内确认弹窗。

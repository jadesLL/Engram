...
1. 首次启动进入启动页：
   ...
   - 或在远端 Web 端「设置 → 多端同步 → 桌面端免密接入」生成令牌，回桌面端填入服务器地址 + 令牌 → 连接。
2. 之后启动会记住上次模式，直接进入。
...
4. 切换模式（进入本地/远端模式后唯一切换途径，快捷键 CmdOrCtrl+Shift+L）：
   ...
   - 或在远端 Web 端「设置 → 多端同步 → 桌面端免密接入」面板点「返回启动页 / 切换模式」（仅桌面端壳内显示该按钮）。

## 远端令牌

在远端服务的 Web 端「设置 → 多端同步 → 桌面端免密接入」生成令牌（默认有效期 365 天，可随时撤销），令牌格式 `lwid_…`。桌面端用它调 `/api/auth/desktop-exchange` 免密兑换登录态，30 天有效，期间所有请求复用现有 cookie 鉴权。

...
- 原始资料/ —— 用户上传的原始文件、用户明确要求保存的 Agent 调研 Markdown 与对话沉积（md/docx/xlsx/pptx/pdf…）。非 md 文件由 Engram 提取文本层；无文字层的 PDF 页保留原样，需你具备视觉能力自行阅读。
  ...
  - **原始资料的正文不写一级标题**：标题由文件名（\`YYYY.MM.DD_标题.md\`）与 frontmatter \`标题\` 承载，界面页头显示的就是它；正文开头再写一行 \`# 标题\` 只是把同一句话重复一遍（新建、save_chat、收集箱入库、create_raw_material 都会去掉开头的一级标题）。
...
- create_raw_material —— 将已完成的调研结果新建为原始资料 Markdown 来源文件；仅在用户明确要求保存调研结果时调用，路径必须是新路径，已有文件拒绝覆盖。**调研成果写 原始资料/文档/、用户随口记的零散内容写 原始资料/灵感碎片/**；正文应保留调研来源与引用，但**不要以一级标题开头**（标题由文件名与 frontmatter 承载，开头的一级标题会被服务端去掉）；原始资料/对话/ 专供 save_chat。收集箱产物由用户在界面确认入库后进入 原始资料/文档/。

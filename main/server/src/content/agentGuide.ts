 ...
 * 名称核验通道（entity_name_check / entity_name_propose / list_entity_names / entity_name_audit）
 * 同样不 bump：它只把「公司全名从哪来」这一步从「查不到就搁置」改成「请示用户后联网核验」，
...
- entity_name_check —— 公司全名核验：登记待核名称（服务端先自查资料库，有候选全名直接返回；没有才请示用户是否允许联网查企查查/天眼查）
- entity_name_propose —— 回填联网查到的工商全名并请示用户是否改用全名（用户同意后由服务端改名）；查不到就不传 fullName
- list_entity_names —— 读名称核验清单（pending 等答复 / open 未办结 / unresolved 最终不是全名）
...
9. **公司全名核验通道（全库唯一允许问用户的事）**：公司类实体（客户/供应商/渠道商/组织）的名称不是工商全名、资料库里也没有全名时，按这条走；除此之外任何信息都不得为此打断用户。
   - **全名的界定**：以「企查查/天眼查等能否查到该名称」为准——能查到工商登记信息才是全名，查到的是简称就继续查全称，查不到就不算。服务端的名称形态判断（是否以「有限公司」等结尾）只是提示，不是结论。
   - **第一步 登记**：entity_name_check（CLI：names check）传 entity（材料里的写法）与 titleOrId（关联页面）。服务端先自查资料库：**页面标题、证据账本、原始资料提取文本，以及正文里写明全名的提法，都算资料库已有全名**，直接返回（用 rename_page 改用全名即可，不打扰用户）；但正文里带「待核实 / 候选 / 疑似」一类存疑标记的写法只算**疑似候选（未核实）**——那正是此前记下还没确认的写法，服务端会把它一并回给你，同时照常登记「是否允许联网查企查查/天眼查」的请示，出现在 Engram 界面「名称核验」。
   - **登记后不要空等**：继续处理下一份资料；下次作业先 list_entity_names（CLI：names list）读用户答复。同一名称只登记一次，服务端会复用未办结的核验。
   - **第二步 联网查询（用户已允许）**：用你自己的联网检索工具查企查查/天眼查，拿到工商登记全名后 entity_name_propose（CLI：names propose）回填 fullName 与 source（出处链接或说明）。服务端随即请示用户「是否把页面标题改为该全名」，**用户同意后由服务端执行改名**（保持页面 ID、重定向引用双链、自动记日志）——你不要自己改名，也不要空等，下次作业读结果。
   - **查不到就不要猜**：不传 fullName 即按「未找到全名」办结。多个同名主体、或查到的是简称：把候选写进 note 让用户判断，不要自行选一个当全名。
   - **用户不允许 / 查不到 / 不同意改名**：标题保持材料写法，名称口径标注「全称待确认」与候选依据，继续作业——**不追问、不反复请示**（服务端也不会重复登记同一名称）。
...
6. **Compose（撰写）**：
   ...
   - 全名从哪来（顺序固定）：① 材料里的工商登记信息（调研报告、年报、出货表、合同抬头）；② 服务端自查——entity_name_check 在页面标题、证据账本、原始资料提取文本里找到全名就直接返回，用 rename_page 改用全名（正文里带「待核实/候选」标记的写法只是疑似候选，未经核实不能当结论）；③ 都没有时走名称核验通道请示用户（操作纪律 9）：用户同意→你联网查企查查/天眼查→entity_name_propose 回填→用户确认后服务端改名。**任何情况下都不得编造或推测全名**；走不通（用户不允许、查不到、不同意改名）时标题暂用材料里的写法，在名称口径记下候选与依据并标注「全称待确认」，后续资料出现全名再用 rename_page（CLI：pages rename）改用全名，不要「新建+删除」。
...
- 公司全名只走名称核验通道（登记→用户允许→联网查→回填→用户确认→服务端改名）：不编造、不推测、不绕过用户自行改名，也不为同一名称反复请示；收尾必须列出「最终不是全名」的条目。

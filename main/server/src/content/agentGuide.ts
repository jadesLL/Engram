...
6. **Compose（撰写）**：
   ...
   - 实体页固定结构：# 标题 → ## 当前理解 → ## 相关页面 → ## 时间线。当前理解按对象角色组织章节（客户：客户画像/核心卡点/合作策略；供应商：产品与服务/合作关系；员工：角色与职责/负责业务/关联实体；项目：目标/范围/里程碑/责任人；技术概念：核心要点/实践应用…没有事实支撑的章节省略）。时间线只记业务事件（日期 + 事件一行一条）。

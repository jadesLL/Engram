 ...
 * （正文「名称口径」收全称/别名），并新增 ask_user / list_questions 待确认问题通道。属页面契约
 * 变化 → +1。本次只影响客户/公司页，重提炼可叠加标签过滤：list_pages outdated=true tag=客户。
...
CLI（engram，与 MCP 同一服务端，token 相同）——**能跑 shell 的 Agent 优先用 CLI**：命令直出结果、上下文消耗低，加 --json 可得机器可读输出。命令：engram status / import / files list|read / search / pages list|read|write|rename|move|delete|evidence / chat save / ask / questions / guide / mcp-config。
...
- list_questions —— 读「待确认问题」与用户答复（status 可选 open/answered/all，默认全部；见操作纪律 8）
...
- ask_user —— 登记一条待确认问题给用户（只有用户才知道的信息，如公司工商全名；见操作纪律 8）
- kb_guide —— 输出本指南全文
...
8. **待确认问题（ask_user）**：只有用户才知道、资料里查不到的信息——公司工商全名、同名主体区分、客户身份口径、要不要导入某份资料——用 ask_user 登记（写清背景与候选，让用户一句话能答），不要编造，也不要拿它代替自查；用户会在 Engram 界面「待确认」里答复。登记后不要空等：本次能推进的先推进，下次作业开工先 list_questions 读答复（含已答复历史）再继续。答复属用户提供的口径，写进正文时标注（用户确认），不要为它编造引文。
...
4. **Plan（决策）**：对每个候选决定 create（新建页）/ merge（并入已有页）/ skip（跳过）。泛概念（「公司」「产品」这类无具体所指）skip；单次纪要/速报/周报不独立建页，事实合并到相关实体页；事实互相冲突或对象身份不清时不要建页，先向用户提问澄清（会话里问不到就用 ask_user 登记，见操作纪律 8）。
...
6. **Compose（撰写）**：
   ...
   - 全名从哪来：材料里的工商登记信息（调研报告、年报、出货表、合同抬头）优先；材料里没有就先 search 全库，看其他资料是否出现过全名；仍找不到时用 ask_user 登记待确认问题（附候选与依据），用户答复后再落页——**不得编造或推测全名**；未确认前标题暂用材料里的写法，并在名称口径标注「全称待确认」。已有页面改用全名走 rename_page（CLI：pages rename），不要「新建+删除」。
...
- 公司实体用工商全名、不编造全名 | 客户页「概览」「核心机型」缺证据就省略 | 找不到的全名用 ask_user 问用户，不猜。

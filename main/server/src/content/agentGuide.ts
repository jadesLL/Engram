...
- create_raw_material —— 将已完成的调研结果新建为 原始资料/ 下的 Markdown 来源文件；仅在用户明确要求保存调研结果时调用，路径必须是新路径，已有文件拒绝覆盖。正文应保留调研来源与引用；原始资料/对话/ 专供 save_chat。收集箱产物由用户在界面确认入库后进入 原始资料/ 根目录。
...
2. **原始资料默认只读，唯一的 Agent 创建入口是 create_raw_material**：只有用户明确要求保存调研结果时，才可用它在 原始资料/ 下新建 Markdown 文件；它拒绝覆盖已有路径，也不能写 原始资料/对话/。收集箱入库必须由用户在界面确认。write_page / rename_page / move_page / delete_page 仍只允许操作 Wiki/ 页面。不得走 HTTP/CLI 旁路写原始资料。用户没要求保存且资料不在库里时，按现有材料推进、把缺口写进页面的「待核实」，不要停下来等用户。

...
import { selectAssistantRuntime } from '../assistant/zcodeRuntime.js';
...
export async function assistantRoutes(app: FastifyInstance) {
  ...
  app.post('/api/assistant/sessions/:id/runs', async (req, reply) => {
    ...
    try {
      const run = selectAssistantRuntime().startRun(id, String(body.message || ''), body.context || {});
  ...
  app.get('/api/assistant/runs/:id', async (req, reply) => {
    const { id } = req.params as { id: string };
    const snapshot = selectAssistantRuntime().snapshotForRun(id);
  ...
  app.post('/api/assistant/runs/:id/decisions', async (req, reply) => {
    ...
    try {
      await selectAssistantRuntime().decideRun(id, decisions || []);
      return { ok: true };
    } catch (error) {
      return reply.code(409).send({ error: errorMessage(error) });
    }
  ...
  app.post('/api/assistant/runs/:id/cancel', async (req, reply) => {
    const { id } = req.params as { id: string };
    try {
      return { run: selectAssistantRuntime().cancelRun(id) };
    } catch (error) {
  ...
  app.post('/api/assistant/runs/:id/retry', async (req, reply) => {
    const { id } = req.params as { id: string };
    try {
      return reply.code(202).send({ run: selectAssistantRuntime().retryRun(id) });
    } catch (error) {
      return reply.code(409).send({ error: errorMessage(error) });
    }
  });
  ...
  app.post('/api/assistant/runs/:id/ingest', async (req, reply) => {
    const { id } = req.params as { id: string };
    try {
      return { meta: await selectAssistantRuntime().ingestRun(id) };
    } catch (error) {
      return reply.code(409).send({ error: errorMessage(error) });
    }
  });
  ...
  app.post('/api/assistant/tool-calls/:id/undo', async (req, reply) => {
    const { id } = req.params as { id: string };
    try {
      await selectAssistantRuntime().undoToolCall(id);
      return { ok: true };
    } catch (error) {
      return reply.code(409).send({ error: errorMessage(error) });
    }

...
export const FORCE_CREATE_NOTICE = '⚠️ 本页由人工强制建立:来源单一,内容未经交叉验证,请谨慎采信。';

function injectForceNotice(content: string): string {
  const body = (content || '').trim();
  return `> ${FORCE_CREATE_NOTICE}\n\n${body}`;
}

/**
 * 快速强制建立:跳过 LLM 局部再提炼,直接用候选已有正文建页。
 * 面向「待入库清单」——客户确认某单来源候选值得立刻建页,不等第二来源自动对账。
 * manualApproval 绕过跨来源门禁;正文开头注入未交叉验证提示块。
 */
export function forceCommitCandidate(
  reportId: number,
  options: { allowApplying?: boolean } = {},
  ...
  const kind = ['concept', 'person', 'customer', 'org', 'place', 'work', 'project', 'other'].includes(candidate.kind)
    ...
    : null;
  if (!kind) throw new Error('候选页面类型无效,无法强制建立');
  const sourceVersion = db.prepare(
    `SELECT id,path,content_hash,previous_id,status FROM source_versions WHERE id=?`
  ).get(candidate.source_version_id) as SourceVersion | undefined;
  if (!sourceVersion || sourceVersion.status !== 'active') {
    throw new Error('候选来源版本已更新,请重新整理最新资料后再处理');
  }
  const target = exactCandidatePage(candidate);
  // 同一实体在其他来源的候选作为证据贡献一并并入页面(evidenceOnly,不影响正文投影),
  // 并随之消耗关闭,避免强制建立后主候选入库而同名其他来源候选残留在清单中。
  const supporting = relatedCandidateOccurrences(candidate)
    .filter((occurrence) =>
      occurrence.id !== candidate.id &&
      occurrence.evidence_eligible &&
  ...
  commitKnowledgeItems([{
    ...
    reason: candidate.reason,
    content: injectForceNotice(candidate.content || candidate.summary),
    candidateId: candidate.id,
    supportingCandidateIds: supporting.map((occurrence) => occurrence.id),
  }], {
    runId: candidate.run_id,
    sourceVersion,
    sourcePath: candidate.source_path,
  ...
  const page = db.prepare(
    `SELECT id,path FROM pages WHERE deleted=0 AND lower(title)=lower(?)
     AND (path LIKE 'Wiki/概念/%' OR path LIKE 'Wiki/实体/%')`
  ).get(candidate.name) as { id: string; path: string } | undefined;
  if (!page) throw new Error('强制建立后未生成知识页面');
  const payload = JSON.parse(report.payload);
  db.prepare(`UPDATE reports SET payload=?,status='resolved' WHERE id=?`).run(
    JSON.stringify({
      ...payload,
  ...
  return { ...page, name: candidate.name, kind };
}

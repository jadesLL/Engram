...
export function mapPrompt(chunkId: string, roster: string): string {
  return `${PERSONA}\n${PRINCIPLES}\n${relationVocabHint()}\n你正在执行 Map。仅抽取当前分段明确支持的长期知识候选和原子事实。每个事实必须带 sources:[{chunkId:"${chunkId}",quote:"原文短引文"}]，quote 必须逐字存在于分段。不要决定建页或合并。当前分段最多输出 16 个候选；如果内容过密，优先保留有明确名称和事实依据的候选，不得输出不完整 JSON。输出 {"candidates":[{"name":"","kind":"concept|person|project|org","domain":"","summary":"","facts":[{"id":"${chunkId}-f1","statement":"","sources":[{"chunkId":"${chunkId}","quote":""}]}],"relations":[]}]}。已有标题仅用于统一命名：\n${roster}\n${JSON_ONLY}`;
}

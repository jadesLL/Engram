 ...
 *
 * 边界：这套门禁只适用于**服务端能用密码校验**的数据操作（密码最终交给接口校验）。
 * 桌面端「卸载 Engram」虽然同样不可撤销、也和它们同组（数据与存储 → 危险操作），
 * 但它走的是本机 IPC、不经服务端，而且必须在内嵌服务起不来 / 用户没设过登录密码时也能用，
 * 所以只用 confirmDialog 确认一次，不要给它加密码门禁（加了会自锁）。
 *
 * 返回：
 *  - null            用户中途取消；

...
export const MCP_TOOLS: McpToolDoc[] = [
  ...
  },
  {
    ...
    group: '读',
    summary: '读取公司全名核验清单：等用户答复、等 Agent 回填、已办结（含「最终不是全名」的条目）。',
    params: 'status pending（等用户答复）/ open（未办结）/ unresolved（最终不是全名）/ all（默认）。',
    notes: '登记核验后下次作业先读这里拿答复；作业收尾用 status=unresolved 把仍未定全名的条目列给用户。',
  ...
  },
  {
    ...
    group: '写',
    summary: '公司全名核验（登记待核名称）：公司名称不是工商全名、资料库里也查不到时，请示用户是否允许联网查企查查/天眼查——全库唯一允许问用户的事。',
    params: 'entity 材料里的名称写法（必填，通常是简称）；titleOrId 关联页面（可选，用户同意改用全名时据此自动改名）；note 说明（可选）。',
    notes: '服务端先自查资料库：页面标题 / 证据账本 / 原始资料提取文本、以及正文里写明全名的提法，都算已有全名，直接返回即可用 rename_page 改名，不打扰用户；正文里带「待核实/候选」标记的写法只作「疑似候选（未核实）」回给你，仍会登记请示。登记后不要空等，下次作业用 list_entity_names 读答复；同一名称只登记一次。',
    cli: 'engram names check <名称> [--page <titleOrId>] [--note <说明>]',
  },
  {
    name: 'entity_name_propose',
    group: '写',
    summary: '回填联网查到的工商全名并请示用户是否改用全名；用户同意后由服务端执行改名（保持页面 ID、双链重定向、自动记日志）。',
